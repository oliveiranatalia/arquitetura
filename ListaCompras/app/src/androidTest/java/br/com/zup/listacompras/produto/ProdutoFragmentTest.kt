...
class ProdutoFragmentTest{
    ...
    fun editTextShowError_whenBothisEmpty(){
        ...
        onView(withId(R.id.etNomeProduto)).check(matches(hasErrorText("Por favor preencha o campo de nome")))
        onView(withId(R.id.etDetalheProduto)).check(matches(hasErrorText("Por favor preencha o campo de detalhe")))
    }

    @Test
    fun detalheShowErrorAndNameDoesnt_whenOnlyDetalheIsEmpty(){
        val scenario = launchFragmentInContainer<ProdutoFragment>()
        onView(withId(R.id.etNomeProduto)).perform(typeText("testando"))
        onView(withId(R.id.bvAdicionar)).perform(click())
        onView(withId(R.id.etDetalheProduto)).check(matches(hasErrorText("Por favor preencha o campo de detalhe")))
